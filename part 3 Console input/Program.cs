...
Console.Clear();
Thread.Sleep(1000);
double legASquared = Math.Pow((double)legA, 2);
double legBSquared = Math.Pow((double)legB, 2);

triangleSum = legASquared + legBSquared;
triangleSum = Math.Sqrt(triangleSum);
triangleSum = Math.Round(triangleSum, 2);
Console.WriteLine("The area of the right triangle is: " + triangleSum + " square units");
